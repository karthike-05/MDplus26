...
export default function ReferralDetail({ referralId, onBack, onReview }) {
  ...
  const { referral, patient, service, attempts } = data;
  // The action widget expects a dashboard-shaped row.
  const row = {
    referral_id: referral.id,
    current_state: referral.current_state,
  ...
  return (
    <div style={s.wrap}>
      ...
      <div style={s.head}>
        ...
        <Badge state={referral.current_state} />
      </div>
...
function summarize(data) {
  if (data.output_path) return `filled ${(data.filled_fields || []).length} fields → ${String(data.output_path).split("/").slice(-2).join("/")}`;
  if (data.intent) return data.intent.replace(/_/g, " ");
  if (data.placed) return "call placed";
